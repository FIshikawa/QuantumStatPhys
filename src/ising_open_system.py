...
if __name__ == '__main__':
    ...
    experimental_condi = []
    experimental_condi.append('Observe Relaxation : ED : Transverse Isin with longitudinal fields ')
    experimental_condi.append('Hamiltonian : J Sz Sz + h Sz + V Sx')
    ...
    if(relaxation):
        ...
        hamiltonian_thermalize.add_H_2body(Sz, Sz, coef=J_int,position=[tagged-1,tagged])
    ...
    hamiltonian_total.add_H_2body(Sz, Sz, coef=J_int,position=[tagged-1,tagged])
    if(engine):
        if(not discrete):
