...
def set_density_matrix():
    N = 2
    pytest.density_matrix = DensityMatrix(N=N)
    pytest.N = N
...
def test_calc(set_density_matrix):
    ...
    assert 0 == pytest.density_matrix.calculate_expectation_of_1body(O=Sz,index=N//2)
    assert 0 == pytest.density_matrix.calculate_expectation_of_1body(O=Sz,index=0)
    assert 0 == pytest.density_matrix.calculate_expectation_of_2body(O1=Sz, O2=Sz, dist=1)

@pytest.fixture(scope='module')
def set_density_matrix_relaxation():
    N = 3
    pytest.density_matrix = DensityMatrix(N=N,tagged=N//2,relaxation=True)
    pytest.N = N
...
def test_calc_relaxation(set_density_matrix_relaxation):
    N = pytest.N
    assert 0 == pytest.density_matrix.calculate_expectation_of_1body(O=Sz)
    assert 1 == pytest.density_matrix.calculate_expectation_of_1body(O=Sx,index=N//2)
    assert 0 == pytest.density_matrix.calculate_expectation_of_1body(O=Sz,index=0)
    assert 0 == pytest.density_matrix.calculate_expectation_of_2body(O1=Sz, O2=Sz, dist=1)
