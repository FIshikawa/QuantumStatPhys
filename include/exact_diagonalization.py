...
class ExactDiagonalization:
    ...
    def __call__(self,density_matrix,t):
        N = self.N
        propagator_left = np.zeros((np.power(2,N),np.power(2,N)),dtype=np.complex128)
        propagator_right = np.zeros((np.power(2,N),np.power(2,N)),dtype=np.complex128)
        ...
        propagator_left = np.matmul(np.matmul(self.P, D_temp) , np.conjugate(self.P.transpose()))
        propagator_right = np.matmul(np.matmul(self.P, D_temp_inv) , np.conjugate(self.P.transpose()))
        rho_temp = np.matmul(np.matmul(propagator_left, density_matrix.density_matrix), propagator_right) 
