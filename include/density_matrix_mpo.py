class DensityMatrixMPO:
  ...
  def __Liouvillian_thermo_self(self, index=-1):
    if(index<0):
      for loop_index in range(self.H_1body_kind):
        self.L_thermo_self[loop_index] = np.zeros((((2, 2, 2, 2))), dtype=np.complex64)
        H_transpose = np.transpose(self.H_1body[loop_index])
        ...
        for i in range(2):
          for j in range(2):
            for k in range(2):
              for l in range(2):
                self.L_thermo_self[loop_index][i][j][k][l] = - 0.5 * ( self.H_1body[loop_index][i][k]*I2[j][l] + I2[i][k]*H_transpose[j][l] )
                #self.L_thermo_self[loop_index][i][j][k][l] = -  self.H_1body[loop_index][i][k]*I2[j][l]

    else:
      if(index >= self.H_1body_kind):
        raise NameError("__Liouvillian_self : index %d must be smaller than %d" % (index, self.H_1body_kind))
      self.L_thermo_self[index] = np.zeros((((2, 2, 2, 2))), dtype=np.complex64)
      H_transpose = np.transpose(self.H_1body[index])
      ...
      for i in range(2):
        for j in range(2):
          for k in range(2):
            for l in range(2):
              self.L_thermo_self[index][i][j][k][l] = - 0.5 * ( self.H_1body[index][i][k]*I2[j][l] + I2[i][k]*H_transpose[j][l] )
              #self.L_thermo_self[index][i][j][k][l] = -  self.H_1body[index][i][k]*I2[j][l] 

  def __Liouvillian_thermo_2body(self, index=-1):
    if(index<0):
      for loop_index in range(self.H_2body_kind):
        self.L_thermo_2body[loop_index] = np.zeros((((4, 4, 4, 4))), dtype=np.complex64)
        H_transpose = np.transpose(self.H_2body[loop_index])
        ...
        for i in range(4):
          for j in range(4):
            for k in range(4):
              for l in range(4):
                #self.L_thermo_2body[loop_index][i][j][k][l] = - self.H_2body[loop_index][i][k]*I4[j][l] 
                self.L_thermo_2body[loop_index][i][j][k][l] = -0.5 * (self.H_2body[loop_index][i][k]*I4[j][l] + I4[i][k]*H_transpose[j][l])
    else:
      if(index >= self.H_2body_kind):
        raise NameError("__Liouvillian_2body : index %d must be smaller than %d" % (index, self.H_2body_kind))
      self.L_thermo_2body[index] = np.zeros((((4, 4, 4, 4))), dtype=np.complex64)
      H_transpose = np.transpose(self.H_2body[index])
      ...
      for i in range(4):
        for j in range(4):
          for k in range(4):
            for l in range(4):
              #self.L_thermo_2body[index][i][j][k][l] = - self.H_2body[index][i][k]*I4[j][l] 
              self.L_thermo_2body[index][i][j][k][l] = -0.5 * (self.H_2body[index][i][k]*I4[j][l] + I4[i][k]*H_transpose[j][l])

  def __Liouvillian_thermo_3body(self, index=-1):
    if(index<0):
      for loop_index in range(self.H_3body_kind):
        self.L_thermo_3body[loop_index] = np.zeros((((8, 8, 8, 8))), dtype=np.complex64)
        H_transpose = np.transpose(self.H_3body[loop_index])
        ...
        for i in range(8):
          for j in range(8):
            for k in range(8):
              for l in range(8):
                #self.L_thermo_3body[loop_index][i][j][k][l] = - self.H_3body[loop_index][i][k]*I8[j][l] 
                self.L_thermo_3body[loop_index][i][j][k][l] = -0.5 *(self.H_3body[loop_index][i][k]*I8[j][l] + I8[i][k]*H_transpose[j][l])
    else:
      if(index >= self.H_3body_kind):
        raise NameError("__Liouvillian_3body : index %d must be smaller than %d" % (index, self.H_3body_kind))
      self.L_3body[index] = np.zeros((((8, 8, 8, 8))), dtype=np.complex64)
      ...
      for i in range(8):
        for j in range(8):
          for k in range(8):
            for l in range(8):
             # self.L_thermo_3body[index][i][j][k][l] = - 0.5 * self.H_3body[index][i][k]*I8[j][l] - 0.5 * I8[i][k]*H_transpose[j][l]
              self.L_thermo_3body[index][i][j][k][l] = -0.5 *(self.H_3body[index][i][k]*I8[j][l] + I8[i][k]*H_transpose[j][l])
  ...
  def initialize_H_1body(self, index=-1):
    ...
    self.__Liouvillian_thermo_self(index)
  ...
  def initialize_H_2body(self, index=-1):
    ...
    self.__Liouvillian_thermo_2body(index)
  ...
  def add_H_2body(self, H_2body1, H_2body2, coef=1.0, index=0):
    ...
    self.__Liouvillian_thermo_2body()
  ...
  def initialize_H_3body(self, index=-1):
    ...
    self.__Liouvillian_thermo_3body(index)
  ...
  def initialize_density_matrix(self,relaxation=False):
    self.p_set = np.zeros(((((self.N, self.D, 2, 2, self.D)))), dtype=np.complex64)
    for n in range(self.N):
      for D1 in range(self.D):
        for D2 in range(self.D):
            if(n != tagged):
              for spin in range(2):
                self.p_set[n][D1][spin][spin][D2] = 1.0
            else:
                if(relaxation):
                   self.p_set[n][D1][0][0][D2] = 1.0
    self.normalize()
  ...
  def density_matrix(self):
    p_mat = np.zeros((np.power(2,self.N),np.power(2,self.N)),dtype=np.complex64) 
    p_set_temp = np.transpose(self.p_set, (0, 2, 3, 1, 4))
    bit_array = (0,1)
    v_right = copy.deepcopy(self.right_edge_vector)
    for n1, spin_config1 in enumerate(itertools.product(bit_array,repeat=self.N),0):
      for n2, spin_config2 in enumerate(itertools.product(bit_array,repeat=self.N),0):
        v_left  = copy.deepcopy(self.left_edge_vector)
        for n in range(self.N):
          v_left = np.dot(v_left, p_set_temp[n][spin_config1[n]][spin_config2[n]])
        p_mat[n1][n2] = np.dot(v_left, v_right) 
  ...
  def operate_thermal_1body(self, particle_index, Liouvillian_index=0):
    #operate 1-body interactions
    if(particle_index < 0 or particle_index >= self.N):
      raise NameError("particle_index %d out of range - operate_1body" % (particle_index))
    if(Liouvillian_index < 0 or Liouvillian_index >= self.H_1body_kind):
      raise NameError("Liouvillian_index %d out of range - operate_1body" % (Liouvillian_index))
    L_thermo_self = copy.deepcopy(self.L_thermo_self[Liouvillian_index])
    L_thermo_self = np.reshape(L_thermo_self, (4, 4))
    L_thermo_self = I4 + L_thermo_self*self.T_delta

    p_tmp = np.transpose(self.p_set[particle_index], (1, 2, 0, 3))
    p_tmp = np.reshape(p_tmp, (2*2, self.D*self.D))
    p_tmp = np.reshape(np.dot(L_thermo_self, p_tmp), (2, 2, self.D, self.D))
    self.p_set[particle_index] = np.transpose(p_tmp, (2, 0, 1, 3))

  def operate_thermal_2body(self, particle_index1, particle_index2, Liouvillian_index=0):
    #operate 2-body interactions
    if(particle_index1 < 0 or particle_index1 >= self.N):
      raise NameError("particle_index %d out of range - operate_2body" % (particle_index1))
    if(particle_index2 < 0 or particle_index2 >= self.N):
    ...
    L_thermo_2body = copy.deepcopy(self.L_thermo_2body[Liouvillian_index])
    L_thermo_2body = np.reshape(L_thermo_2body, (4*4, 4*4))
    ...
    L_thermo_2body = I16 + L_thermo_2body*self.T_delta 
    L_thermo_2body = np.dot(L_thermo_2body, P)

    L_thermo_2body = np.reshape(L_thermo_2body, (2, 2, 2, 2, self.D, self.D))
    L_thermo_2body = np.transpose(L_thermo_2body, (4, 0, 2, 1, 3, 5))
    L_thermo_2body = np.reshape(L_thermo_2body, (self.D*2*2, 2*2*self.D))

    p_tmp1, p_tmp2 = self.__decompose_L_2body_to_p(L_thermo_2body)

    self.p_set[particle_index1] = np.reshape(p_tmp1, (self.D, 2, 2, self.D))
    self.p_set[particle_index2] = np.reshape(p_tmp2, (self.D, 2, 2, self.D))

  def operate_thermal_3body(self, particle_index1, particle_index2, particle_index3, Liouvillian_index=0):
    #operate 3-body interactions
    if(particle_index1 < 0 or particle_index1 >= self.N):
      raise NameError("particle_index %d out of range - operate_3body" % particle_index1)
    if(particle_index2 < 0 or particle_index2 >= self.N):
    ...
    L_thermo_3body = copy.deepcopy(self.L_thermo_3body[Liouvillian_index])
    L_thermo_3body = np.reshape(L_thermo_3body, (4*4*4, 4*4*4))
    ...
    L_thermo_3body = I64 + L_thermo_3body*self.T_delta 
    L_thermo_3body = np.dot(L_thermo_3body, P)

    L_thermo_3body = np.reshape(L_thermo_3body, (2, 2, 2, 2, 2, 2, self.D, self.D))
    L_thermo_3body = np.transpose(L_thermo_3body, (6, 0, 3, 1, 4, 2, 5, 7))
    L_thermo_3body = np.reshape(L_thermo_3body, (self.D*2*2, 2*2*2*2*self.D))

    p_tmp1, p_tmp2, p_tmp3 = self.__decompose_L_3body_to_p(L_thermo_3body)

    self.p_set[particle_index1] = np.reshape(p_tmp1, (self.D, 2, 2, self.D))
    self.p_set[particle_index2] = np.reshape(p_tmp2, (self.D, 2, 2, self.D))
    self.p_set[particle_index3] = np.reshape(p_tmp3, (self.D, 2, 2, self.D))

class MPO_1D_Tagged(MPO_1D):
  def __init__(self, N, D, T_delta, tagged, H_1body_kind=1, K_1body_kind=1, H_2body_kind=1, H_3body_kind=1,test_mode=False):
    MPO_1D.__init__(self,N=N, D=D, T_delta=T_delta , H_1body_kind=H_1body_kind+1, K_1body_kind=K_1body_kind, H_2body_kind=H_2body_kind+1, H_3body_kind=H_3body_kind+1)
    self.initialize_H_1body()
    self.initialize_H_2body()
    self.initialize_H_3body()
    self.tagged = tagged
    if(not test_mode):
      self.add_H_1body(I, 1.0,H_1body_kind-1)
      self.add_H_2body(I, I, 1.0,H_2body_kind-1)
      self.add_H_3body(I, I, I, 1.0,H_3body_kind-1)
    

  def total_thermalize_step_without_tagged(self):
    #operate 1-body interactions
    for index in range(self.H_1body_kind-1):
      for n in range(self.N):
        if( n != self.tagged):
          self.operate_thermal_1body(n,index)
        else:
          self.operate_thermal_1body(n,self.H_1body_kind-1)
        
    #operate 2-body interactions
    for index in range(self.H_2body_kind-1):
      for n in range(self.N//2):
        if( 2*n != self.tagged and (2*n+1) != self.tagged):
          self.operate_thermal_2body(2*n, 2*n+1,index)
        else:
          self.operate_thermal_2body(2*n, 2*n+1,self.H_2body_kind-1)

      for n in range((self.N-1)//2):
        if( (2*n+1) != self.tagged and (2*n+2) != self.tagged):
          self.operate_thermal_2body(2*n+1, 2*n+2,index)
        else:
          self.operate_thermal_2body(2*n+1, 2*n+2,self.H_2body_kind-1)
    self.normalize()

  def total_thermalize_step(self):
    #operate 1-body interactions
    for index in range(self.H_1body_kind-1):
      for n in range(self.N):
        self.operate_thermal_1body(n,index)
  ...
  def total_time_step(self):
    ...
    for index in range(self.H_1body_kind-1):
      for n in range(self.N):
        self.operate_1body(n,index)

      #operate 2-body interactions
    for index in range(self.H_2body_kind-1):
      for n in range(self.N//2):
        self.operate_2body(2*n, 2*n+1,index)
      for n in range((self.N-1)//2):
        self.operate_2body(2*n+1, 2*n+2,index)
    self.normalize()
