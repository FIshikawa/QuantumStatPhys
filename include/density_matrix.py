...
class DensityMatrix:
    def __init__(self, N, tagged=-1,relaxation=False,Sx_init=1.0):
        self.density_matrix = np.zeros((np.power(2,N),np.power(2,N)),dtype=np.complex128)
        self.tagged = tagged
        self.N = N
        if(Sx_init > 1):
    ...
    def initialize(self,relaxation=False):
        if(relaxation):
            ...
            density_matrix_left = np.eye(np.power(2,self.N-self.tagged-1),dtype=np.complex128)
            density_matrix_tagged = 0.5 * np.eye(2,dtype=np.complex128) + 0.5 * self.Sx_init * Sx
            density_matrix = np.kron(density_matrix_left,np.kron(density_matrix_tagged,density_matrix_right))
        self.density_matrix = density_matrix / np.trace(density_matrix)
  
    def calculate_expectation_of_1body(self, O=Sz, index=-1):
        N = self.N
        operator_total= np.zeros((np.power(2,N), np.power(2,N)), dtype=np.complex128)
        if(index == -1):
            for i in range(N):
    ...
    def calculate_expectation_of_2body(self, O1=Sz, O2=Sz, dist=1, index=-1):
        ...
        if(index == -1):
            for i in range(N-dist-1):
                ...
                operator_left = np.eye(np.power(2,N-i-2-dist),dtype=np.complex128)
            ...
            operator_left = np.eye(np.power(2,N-index-1-dist),dtype=np.complex128)
            operator_temp = np.kron(O1,operator_right)
            operator_temp = np.kron(operator_middle,operator_temp)
            operator_temp = np.kron(O2,operator_temp)
            operator_temp = np.kron(operator_left,operator_temp)
            ...
            raise ValueError("index {} and dist {} is over whole size".format(index, dist))
    ...
    def reduced_density_matrix(self, left, right):
        ...
        if(left < 0 or N-1 < right):
            raise ValueError('area {} to {} is over defined system [0 , N-1]'.format(left,right))
        density_matrix = self.density_matrix.copy()
        self.reduce_left = left 
        self.reduce_right = right
        area_size = right - left + 1 
        reduced_density = np.zeros((np.power(2,area_size),np.power(2,area_size)),dtype=np.complex128)
        ...
        for i in range(len(matrix_basis)):
            matrix_basis[i] = 1
            matrix_basis = np.reshape(matrix_basis,(np.power(2,area_size),np.power(2,area_size)))
            total_reduce_basis = np.kron(left_reduction,np.kron(matrix_basis,right_reduction))
            reduced_density += np.trace(np.dot(density_matrix,total_reduce_basis)) * matrix_basis
